...
const Laptop = () => {
  ...
  const laptopCate = laptopCategory();

  const laptopData = laptopCate.slice(0, loadMore)
  const handleLoadMore = () => {
    setLoadMore(loadMore + 10)
  }
  useEffect(() => {
    if(laptopCate.length - loadMore <= 0) {
      setEndLoad(true)
    }
  }, [loadMore])
  return (
    <div className='pt-44 px-4 lg:px-8 xl:mx-[10%] 2xl:mx-[16%]'>
      <div className='flex gap-2 items-center'>
          <p className='text-[#115E5C] text-xl font-bold'>Laptop</p>
        </div>
        <div>
        ...
        { endLoad === false && (
          <div className='flex justify-center w-full mt-8'>
            ...
            >
              Xêm thêm {laptopCate.length - loadMore} laptop
            </button>
          </div>
        )}
    </div>
