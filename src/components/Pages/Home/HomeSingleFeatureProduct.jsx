    ...
    <div>
        {singlePeatureProduct && (
            singlePeatureProduct.map(item => (
                <div key={item.id} className='pt-32 bg-white lg:flex lg:bg-[#f6f6f6] lg:px-4 xl:mx-[10%] 2xl:mx-[16%]'>
                    ...
                    <div className='px-4 flex flex-col bg-[#f6f6f6] lg:w-1/2'>
                        ...
                        <div className='flex items-center gap-4 my-6' >
                            ...
                            <div className='flex items-center flex-wrap gap-3'>
                                <h2 className='font-bold'>{new Intl.NumberFormat('it-IT', {style : 'currency', currency : 'VND'}).format(item.price * quantity)}</h2>
                                <p className='text-gray-400 line-through'>{new Intl.NumberFormat('it-IT', {style : 'currency', currency : 'VND'}).format(item.price * item.old_price * quantity)}</p>
                            </div>
                        </div>
                        ...
                        <div className="my-4 pt-4">
                            <p className='font-normal text-[14px] text-justify text-[#0000008f]'>{item?.description}</p>
